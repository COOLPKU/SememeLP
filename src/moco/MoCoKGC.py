...
class DualSememeFusion(nn.Module):
    def __init__(self, embed_dim=768):
        ...
        self.balance_layer = nn.Sequential(
            nn.Linear(2*self.embed_dim, self.embed_dim*4),
            nn.GELU(),
            nn.Dropout(0.4),
            nn.Linear(self.embed_dim*4, 2),  # 输出 main/all 的平衡权重
            nn.Softmax(dim=-1)
        )
        
        # FGF
        self.joint_fuse = nn.Sequential(
            nn.Linear(4*self.embed_dim, self.embed_dim*8),
            nn.GELU(),
            nn.Dropout(0.4),
    ...
    def _build_gate(self, embed_dim):
    ...
    def forward(self, text_embed, main_sememe, all_sememe):
        ...
        # === IGF ===
        # Main
        combined_main = torch.cat([text_embed, main_sememe], dim=-1)
        gate_main = self.gate_main(combined_main)
        fused_main = gate_main * text_embed + (1 - gate_main) * main_sememe  # [batch, dim]
        
        # All
        combined_all = torch.cat([text_embed, all_sememe], dim=-1)
        gate_all = self.gate_all(combined_all)
        fused_all = gate_all * text_embed + (1 - gate_all) * all_sememe  # [batch, dim]

        # === WF ===
        balance_weights = self.balance_layer(
            torch.cat([fused_main, fused_all], dim=-1)
        )  # [batch, 2]
        w_main, w_all = balance_weights.unbind(dim=-1)  # 各 [batch]
        
        balanced = w_main.unsqueeze(-1) * fused_main + w_all.unsqueeze(-1) * fused_all  # [batch, dim]

        # === FGF ===
        joint_input = torch.cat([text_embed, balanced, fused_main, fused_all], dim=-1)
        final_gate = self.joint_fuse(joint_input)
        
        return final_gate * text_embed + (1 - final_gate) * balanced
...
class MoCo_encoder(nn.Module):
    ...
    def __init__(self, args, entities_size, relations_size, NeighborsRepresentationSets, method='MLP', hr_encoder=False):
        '''
        ...
        self.dualSememeFusion = DualSememeFusion(self.hidden_size)
...
class MoCoKGC(nn.Module):
    ...
    def __init__(self, args, entities_size, relations_size, NeighborsRepresentationSets=None, num_training_steps=None):
        ...
        self.hr_encoder:MoCo_encoder = MoCo_encoder(args, entities_size, relations_size, NeighborsRepresentationSets, method=args.entity_embedding_method, hr_encoder=True)
        self.e_encoder:MoCo_encoder = MoCo_encoder(args, entities_size, relations_size, NeighborsRepresentationSets, method=args.entity_embedding_method)
        self.momentum_e_encoder:MoCo_encoder = MoCo_encoder(args, entities_size, relations_size, NeighborsRepresentationSets, method=args.entity_embedding_method)
