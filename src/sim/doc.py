...
class Example:
    ...
    def vectorize(self,hw_tokenizer=None) -> dict:
        ...
        head_hw_text = ' '.join(['[CLS]'] + prompt_tokens + [head_desc] + ['[SEP]'])
        head_hw_encoding = hw_tokenizer(
            head_hw_text,
            add_special_tokens=False,  # 已经手动添加
            max_length=64,
            truncation=True,
            )
        tail_hw_text = ' '.join(['[CLS]'] + prompt_tokens + [tail_desc] + ['[SEP]'])
        tail_hw_encoding = hw_tokenizer(
            tail_hw_text,
            add_special_tokens=False,  # 已经手动添加
            max_length=64,
            truncation=True,
        ...
        return {'hr_token_ids': hr_encoded_inputs['input_ids'],
                ...
                'obj': self,
                'head_hw_token_ids' : head_hw_encoding['input_ids'],
                'tail_hw_token_ids' : tail_hw_encoding['input_ids']
               }


class Dataset(torch.utils.data.dataset.Dataset):

    def __init__(self, path, task,hw_tokenizer, examples=None):
    ...
    def __getitem__(self, index):
        return self.examples[index].vectorize(self.hw_tokenizer)
...
def collate(batch_data: List[dict]) -> dict:
    ...
    tail_token_type_ids = to_indices_and_mask(
        ...
        need_mask=False)
    #print(batch_data[0]['tail_token_type_ids'].size())
    #print(batch_data[0]['tail_hw_token_ids'].size())
    tail_hw_token_ids, tail_hw_mask = to_indices_and_mask(
        [torch.LongTensor(ex['tail_hw_token_ids']) for ex in batch_data],
        pad_token_id=get_tokenizer().pad_token_id)
    ...
    head_hw_token_ids, head_hw_mask = to_indices_and_mask(
        [torch.LongTensor(ex['head_hw_token_ids']) for ex in batch_data],
        pad_token_id=get_tokenizer().pad_token_id)
    ...
    batch_dict = {
        ...
        'tail_hw_token_ids': tail_hw_token_ids,
        'tail_hw_mask': tail_hw_mask,
        'head_hw_token_ids': head_hw_token_ids,
        'head_hw_mask': head_hw_mask,
    }
