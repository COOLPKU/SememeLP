...
def eval_single_direction(predictor: BertPredictor,
    ...
    for idx, ex in enumerate(examples):
        ...
        cur_score_info = {entity_dict.get_entity_by_idx(topk_idx).entity_id: round(topk_score, 3)
                          for topk_score, topk_idx in zip(cur_topk_scores, cur_topk_indices)}

        pred_info = PredInfo(head=ex.head,head_id=ex.head_id,head_desc=ex.head_desc, relation=ex.relation,
                             tail=ex.tail,tail_id=ex.tail_id,tail_desc=ex.tail_desc,pred_tail=entity_dict.get_entity_by_idx(pred_idx).entity,pred_tail_id=entity_dict.get_entity_by_idx(pred_idx).entity_id,pred_desc=entity_dict.get_entity_by_idx(pred_idx).entity_desc,
